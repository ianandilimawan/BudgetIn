...
@section('title', 'Transaction Category Details')
...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between mb-6">
        <div class="flex items-center gap-4">
            <a href="{{ route('admin.transaction_categories.index') }}" class="p-2 bg-white dark:bg-gray-800 rounded-full shadow-sm border border-gray-100 dark:border-gray-700 text-gray-500 hover:text-indigo-600 hover:bg-gray-50 transition-all duration-200">
            ...
            <div>
                <nav class="flex text-sm text-gray-500 font-medium mb-1" aria-label="Breadcrumb">
                    <ol class="inline-flex items-center space-x-1 md:space-x-2">
                        <li class="inline-flex items-center">
                            <a href="{{ route('admin.transaction_categories.index') }}" class="hover:text-indigo-600 transition-colors">Transaction Categories</a>
                        </li>
                        <li>
                ...
                <h1 class="lg:text-2xl text-xl font-semibold text-gray-900 dark:text-white">Transaction Category Details</h1>
            </div>
        </div>

        @if (auth()->user() && auth()->user()->hasPermission('edit-transaction_categories'))
            <a href="{{ route('admin.transaction_categories.edit', $transactionCategory) }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 transition-colors text-sm font-semibold shadow-sm hover:shadow-md">
            ...
            <div class="border border-gray-100 dark:border-gray-800 rounded-xl overflow-hidden divide-y divide-gray-100 dark:divide-gray-800">
                <dl class="mb-0">
                    ...
                    <div class="lg:px-6 px-4 lg:py-4 py-3 flex flex-col lg:flex-row lg:items-center hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                        ...
                        <dd class="mt-1 lg:mt-0 lg:w-2/3 text-sm text-gray-900 dark:text-white font-medium flex items-center gap-3">
                            {!! \App\Helpers\CategoryIconHelper::renderBadge($transactionCategory->icon, $transactionCategory->type ?? 'expense', 'w-10 h-10') !!}
                            <span class="text-sm font-semibold text-zinc-800 dark:text-zinc-200">
                                {{ \App\Helpers\CategoryIconHelper::getIconDefinitions()[$transactionCategory->icon]['label'] ?? $transactionCategory->icon ?? 'Umum' }}
                            </span>
                        </dd>
                    </div>
                    <div class="lg:px-6 px-4 lg:py-4 py-3 flex flex-col lg:flex-row lg:items-center hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                    ...
                    <div class="lg:px-6 px-4 lg:py-4 py-3 flex flex-col lg:flex-row lg:items-center hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                        ...
                        <dd class="mt-1 lg:mt-0 lg:w-2/3 text-sm text-gray-900 dark:text-white font-medium">
                            {{ $transactionCategory->created_at->format('M d, Y H:i') }}
                        </dd>
                    </div>
                    ...
                    <div class="lg:px-6 px-4 lg:py-4 py-3 flex flex-col lg:flex-row lg:items-center hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                        ...
                        <dd class="mt-1 lg:mt-0 lg:w-2/3 text-sm text-gray-900 dark:text-white font-medium">
                            {{ $transactionCategory->updated_at->format('M d, Y H:i') }}
                        </dd>
                    </div>
                </dl>
            </div>
