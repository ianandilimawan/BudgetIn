...
<div class="space-y-4 sm:space-y-5 pb-6">
    ...
    <div class="flex items-center justify-between gap-3">
        <div>
            <h1 class="text-base sm:text-lg md:text-xl font-bold tracking-tight text-zinc-900 dark:text-white">Tambah Jadwal Transaksi Rutin</h1>
            <p class="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">Buat jadwal otomatis untuk mencatat pengeluaran atau pemasukan rutin secara otomatis.</p>
        </div>
    ...
    <div class="bg-white dark:bg-zinc-900 border border-zinc-200/80 dark:border-zinc-800 rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-xs">
        <form action="{{ route('admin.recurring_transactions.store') }}" method="POST">
            @csrf
